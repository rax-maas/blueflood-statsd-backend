 ...
 * @param {Number} retry 
 */
function postRaw(options, transport, payload, log, retry, auth) {
  ...
  var request = transport.request(options, function(response) {
    ...
    if (response.statusCode === 401) {
      log.log('Received 401. Will attempt authentication');
      auth.authenticate(function(err, token) {
        if(token != null || typeof token != 'undefined') {
          options.headers['X-Auth-Token'] = token
          // retry with proper auth.
          log.log('Retrying request with new token.');
          ...
          var err = new Error('Reauthentication failed. Check credentials.');
          var message = JSON.stringify(err, ['stack', 'message'], 2)
          log.log(message)
        }
      });
  ...
  Object.keys(payloadObj).forEach(function(label) {
    payloadObj[label].forEach(function(metric) {
      ...
      if (curJsonSize + objLength > realMax) {
        if(curJsonSize > realMax) {
          // Since current payload is bigger than max allowed. It won't be sent to blueflood. Logging this as error.
          // At the same time, process continues, we don't want to halt the metrics-flow
          Logger.error(
            "json-size " + curJsonSize + " exceeds max value of " + realMax + ". " +
            "Error Payload is : " + JSON.stringify(basePayload)
          );
        }
        else {
          // send and start over.
...
function postMetrics(
  ...
  tenantId, gauges, counters, timers, sets, flushInterval, maxJsonSize, auth) {
  log.log("Posting metrics to blueflood");

  var payloads = buildPayloads(tenantId, timestamp, gauges, counters, timers, sets, flushInterval,
