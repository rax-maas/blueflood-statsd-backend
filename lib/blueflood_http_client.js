...
function reAuthenticate(tenantId,apiKey,username){
    token= ""
	...
	var req = https.request(options,function(res) {
	  ...
	  res.on('data', function(data) {
		console.log(res.statusCode+"Response code");
		responseString += data;
	  });

	  res.on('end', function() {
	  	  console.log(res.statusCode+"End event reached");
...
function postMetrics(
  ...
  tenantId, apiKey, gauges, counters, timers, sets,
  flushInterval) {
  
  reAuthenticate(tenantId,"89ecb97253191d56c1689c0d019c55b6","corpdev0paxon0");
