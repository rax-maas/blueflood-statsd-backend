 ...
 * @param retry
 */
function postRaw(options, transport, payload, log, retry, store_token_callback) {
	  ...
	  if (response.statusCode == 401) {
		var reauth_callback = function (token) {
			...
			if(token != null || typeof token != 'undefined') {
				...
				log.log(response.statusCode + ' No token received! Error!'); //TODO: Raise exception?
			}
 		}		
		authenticate(this.tenantId,"89ecb97253191d56c1689c0d019c55b6","corpdev0paxon0",reauth_callback);
	  }
...
function postMetrics(
  ...
  flushInterval,store_token_callback) {
 ...
 var postmetrics = function(token) { 
    ...
    // kinda hacky, but hey...
    	...
    	postRaw(options, transport, payload, log, 0,store_token_callback);
  	});
  }

  if (auth_token === null || typeof auth_token === 'undefined') {
	  authenticate(this.tenantId,"","",postmetrics);
  }
  else {
