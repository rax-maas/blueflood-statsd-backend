  ...
  var request = transport.request(options, function(response) {
    response.on('data', function(data) {
      if (response.statusCode == 401) {
        rax_authenticate(username,apiKey,function (error, token) {
          if(token != null || typeof token != 'undefined') {
            ...
            var err = new Error('Reauthentication failed: '+error);
            var message = JSON.stringify(err, ['stack', 'message'], 2)
            console.error(message)
          }
          else {
            var err = new Error('Reauthentication failed. Check raxusername and raxapikey.');
            var message = JSON.stringify(err, ['stack', 'message'], 2)
            console.error(message)
          }
        });
...
function rax_authenticate(username,apikey,callback) {
  var token = null,
    authjson = {
      "auth": {
...
function postMetrics(
  ...
  tenantId, gauges, counters, timers, sets, flushInterval, username, apiKey, maxJsonSize) {
  console.log("Posting metrics to blueflood");

  var postmetrics = function(error, token) {
    if (error) {
      if (log) {
        log.log('Error: ' + error);
      }
  ...
  if (username && (auth_token === null || typeof auth_token === 'undefined')) {
    rax_authenticate(username,apiKey,postmetrics);
  }
  else {
    postmetrics(null,auth_token);
  } 
}
