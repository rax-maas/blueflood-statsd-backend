...
function StaticTenantBackend(tenantId, endpoint,flushInterval, log,username, apikey) {
...
StaticTenantBackend.prototype.flush = function (timestamp, metrics) {
  ...
  var gauges = metric_utils.extractGauges(metrics),
     ...
     blueflood_http_client.postMetrics(this.endpoint, this.log, timestampInMillis, this.tenantId, gauges, counters, timers, sets, this.flushInterval, this.username, this.apiKey);
}
