...
var logAll = false;
var cmendpoint = "http://staging.metrics-ingest.api.rackspacecloud.com"
...
function bluefloodConfigOk(conf, logger) {
  ...
  if (!conf.hasOwnProperty('raxusername')) {
    logger.log('Missing BF API key', 'LOG_ERR');
    return false;
  }

  if (!conf.hasOwnProperty('raxapikey')) {
    logger.log('Missing BF API key', 'LOG_ERR');
    return false;
  }
...
exports.init = function (startupTime, config, events, logger) {
  ...
  if (bfConfig.raxusername != null && bfConfig.raxapikey != null) {
  	endpoint = cmendpoint;
  }
  globalBackend = new StaticTenantBackend(bfConfig.tenantId, endpoint, flushIntervalMillis, realLogger, bfConfig.raxusername, bfConfig.raxapikey);
